...
Реализация аналога unix pipeline, что-то наподобие:

```bash
cat common.go | grep func | sort | uniq -c | sort -n  
```

когда STDOUT одной программы передается как STDIN в другую программу, но в нашем случае эти роли выполняют каналы, которые мы передаем из одной функции в другую.
Если кратко, то запускается следующая цепочка:
...
Реализация написана в [spammer.go](./spammer.go)
